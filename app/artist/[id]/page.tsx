...
export default function ArtistPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchAll = async () => {
    ...
    if (artistRes.data) {
      ...
      // Fetch Audio Track based on artist name
  ...
  return (
    <main className="min-h-screen bg-[#020202] text-white selection:bg-[#37C561]/30 overflow-x-hidden relative">
      ...
      {/* Glass Navigation */}
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4 backdrop-blur-md border-b border-white/5 bg-black/20">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <Link href="/" className="flex items-center gap-2 group">
            ...
            <span className="font-black text-[10px] tracking-widest uppercase text-zinc-500 group-hover:text-zinc-200 transition-colors">{lang === 'KO' ? '순위로 돌아가기' : 'Return to Rankings'}</span>
          </Link>
          <div className="flex items-center gap-4">
             <LanguageSwitcher lang={lang} onToggle={() => setLang(prev => prev === 'EN' ? 'KO' : 'EN')} />
             <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-neon-lime rounded-full shadow-[0_0_8px_#bcfe00]" />
                <span className="font-black text-[10px] tracking-tighter uppercase text-neon-lime">System Online</span>
             </div>
          </div>
        </div>
      </nav>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-8">
              <div>
                ...
                <div className="flex gap-4">
                  <div className="glass-panel px-6 py-3 flex flex-col gap-1">
                    <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">{t('globalPower')}</span>
                    <span className="text-3xl font-black italic text-neon-lime">{(artist.total_votes || 0).toLocaleString()}</span>
                  </div>
                  <div className="glass-panel px-6 py-3 flex flex-col gap-1">
                    <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">{t('coverage')}</span>
                    <span className="text-3xl font-black italic text-neon-cyan">{countryStats.length} <span className="text-sm not-italic opacity-50">{t('regions')}</span></span>
                  </div>
                  <button 
                    onClick={handleShare}
                    className="glass-panel px-6 py-3 flex flex-col items-center justify-center gap-1 group/share hover:border-neon-cyan/50 transition-all border-white/10"
                  >
                    <Share2 size={16} className="text-neon-cyan group-hover/share:scale-110 transition-transform" />
                    <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">{t('shareSync')}</span>
                  </button>
                </div>
              </div>
              ...
              {topTrack ? (
                ...
                >
                  ...
                  <div className="flex items-center gap-6 relative z-10">
                    <div className="w-20 h-20 rounded-xl overflow-hidden border border-white/10 shadow-lg relative">
                      ...
                      >
                        {isPlaying ? <Pause fill="white" size={32} /> : <Play fill="white" size={32} className="translate-x-0.5" />}
                      </button>
                    </div>
                    <div className="flex-1">
                      ...
                      <h3 className="text-lg font-black truncate mb-1">{topTrack.title}</h3>
                      <p className="text-xs text-zinc-500 font-bold">{lang === 'KO' ? '대표곡 미리듣기' : 'Representative Track Preview'}</p>
                    </div>
                    <div className={`w-12 flex flex-col gap-1 pr-2 ${isPlaying ? 'opacity-100' : 'opacity-20'}`}>
                      {[1, 2, 3, 4].map(i => (
                ...
                >
                  ...
                  <div>
                    <p className="text-[10px] font-black text-zinc-500 tracking-widest uppercase mb-1">{t('audioUnavailable')}</p>
            ...
            <div className="relative max-w-md mx-auto lg:mx-0 lg:ml-auto">
               ...
               {/* Integrated Minimal Governance (Under Photo) */}
               <div className="mt-6 flex justify-center gap-4 px-2 relative z-10">
                 <button 
                   onClick={() => setIsEditModalOpen(true)}
                   className="flex items-center gap-2 px-6 py-2 rounded-full bg-white/5 border border-white/10 hover:border-[#37C561]/50 hover:bg-[#37C561]/5 transition-all group/edit"
                 >
                   <Edit2 size={12} className="text-zinc-500 group-hover/edit:text-[#37C561] transition-colors" />
                   <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500 group-hover/edit:text-[#37C561] transition-colors">{t('curatePhoto')}</span>
                 </button>
                 <button 
                   onClick={() => setIsReportModalOpen(true)}
                   className="flex items-center gap-2 px-6 py-2 rounded-full bg-white/5 border border-white/10 hover:border-red-500/50 hover:bg-red-500/5 transition-all group/flag"
                 >
                   <Flag size={12} className="text-zinc-500 group-hover/flag:text-red-500 transition-colors" />
                   <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500 group-hover/flag:text-red-500 transition-colors">{t('reportNode')}</span>
                 </button>
               </div>
              </div>
            </div>
      ...
      {/* Dashboard Grid */}
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-12 gap-8 pb-24">
        
        {/* Analytics Scanner (Left 7) */}
        <section className="lg:col-span-7 space-y-6">
           <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-black italic flex items-center gap-3">
                <Globe2 className="text-neon-cyan" size={24} />
                {t('fandomDensity')}
              </h2>
              <div className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-[8px] font-black text-zinc-500 uppercase tracking-widest">
                {lang === 'KO' ? '데이터 갱신: 실시간' : 'Data Refresh: Real-time'}
              </div>
           </div>
           ...
           <div className="glass-panel p-8 min-h-[500px] relative overflow-hidden">
              ...
              {countryStats.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-zinc-600 gap-4">
                   ...
                   <p className="font-black text-sm uppercase tracking-widest">{lang === 'KO' ? '해당 지역의 데이터가 아직 없습니다' : 'No Fandom Patterns Detected In This Region'}</p>
                    ...
                    >
                      <div className="flex justify-between items-end mb-2">
                        <div className="flex items-center gap-3">
                           <span className="font-mono text-[10px] text-zinc-600 w-4 font-black">0{i+1}</span>
                           <span className="text-2xl">{s.flag}</span>
                           <span className="font-black text-sm tracking-tight group-hover:text-neon-cyan transition-colors uppercase">{s.name}</span>
                        </div>
        ...
        <section className="lg:col-span-5 space-y-6">
           <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-black italic flex items-center gap-3">
                <MessageCircle className="text-neon-magenta" size={24} />
                {t('liveHub')}
              </h2>
           </div>
                ...
                {user ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-2 gap-3 border-b border-white/10 pb-4">
                    <div className="flex items-center gap-2 text-[10px] font-black text-zinc-500 uppercase tracking-widest">
                       <ShieldAlert size={14} className="text-neon-magenta/50" />
                       Unauthorized Access Detected
                    </div>
                    <Link 
                      href="/login" 
                      className="px-6 py-2 bg-neon-magenta/20 border border-neon-magenta/50 text-neon-magenta rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-neon-magenta hover:text-white transition-all shadow-[0_0_15px_rgba(255,0,255,0.1)]"
                    >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                     ...
                     <button
                        ...
                        className="px-6 py-2 rounded-lg bg-neon-magenta text-white font-black text-[10px] uppercase tracking-[0.2em] hover:bg-magenta-600 transition-all active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed shadow-[0_0_10px_rgba(255,0,255,0.3)]"
                      >
                        {t('transmit')}
                      </button>
                  </div>
                ...
                <AnimatePresence initial={false} mode="popLayout">
                  {comments.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-zinc-700 gap-3 grayscale opacity-30">
                       <Send size={32} />
                       <p className="font-black text-[10px] uppercase tracking-widest">{lang === 'KO' ? '첫 번째 메시지를 기다리고 있습니다...' : 'Awaiting First Transmission...'}</p>
                    </div>
                  ) : (
                    comments
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex items-center gap-2">
                            <span className="w-6 h-6 rounded-md bg-zinc-800 flex items-center justify-center text-xs border border-white/10 group-hover:border-neon-magenta/50 transition-colors">
                              {c.country_code ? (COUNTRY_FLAGS[c.country_code] ?? '🌐') : '💜'}
                            </span>
                            <span className="text-[10px] font-black uppercase text-zinc-400 group-hover:text-neon-magenta transition-colors">{c.display_name || 'Anonymous Fan'}</span>
                          </div>
