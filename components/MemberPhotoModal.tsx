...
import { X, Image as ImageIcon, CheckCircle2, AlertCircle, Loader2, Globe, Search } from 'lucide-react';
...
export default function MemberPhotoModal({
  ...
  // Combine member name and artist name for better search accuracy
  const [searchQuery, setSearchQuery] = useState(`${memberName} ${artistName}`);
            ...
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/5">
              <div>
                <h3 className="text-lg font-black italic uppercase tracking-tighter flex items-center gap-2">
                  ...
                  {t('memberPhotoModalTitle')}: <span className="text-neon-magenta font-chakra">{memberName}</span>
                </h3>
                <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-1">{t('memberPhotoModalSub')}</p>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-8">
              {errorStatus ? (
                <div className="text-center space-y-6 flex flex-col items-center py-10">
                  ...
                  <button onClick={() => setErrorStatus(null)} className="w-full max-w-xs py-4 rounded-2xl border border-white/10 font-black text-[10px] uppercase tracking-widest text-neon-magenta">
                    BACK
                  </button>
                </div>
              ) : (
                <>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        className="w-full bg-black/40 border border-white/10 rounded-2xl pl-12 pr-4 py-4 text-sm font-bold text-white placeholder:text-zinc-800 focus:border-neon-magenta/50 outline-none transition-all"
                      />
                      {loadingResults && <Loader2 size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-neon-magenta animate-spin" />}
                    </div>
                          ...
                          {results.map((res) => (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest mt-0.5">READY_TO_SYNC</p>
                              </div>
                            </button>
                          ))}
                        </motion.div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <div className="aspect-square w-full max-w-[200px] rounded-full overflow-hidden border-2 border-white/10 bg-black/40 relative shadow-2xl">
                        {url && !previewError ? (
            ...
            {!errorStatus && (
              <div className="p-8 bg-black/40 border-t border-white/5 flex gap-4">
                ...
                <button
                  ...
                  className="flex-[2] py-4 rounded-3xl bg-neon-magenta text-white font-black text-[10px] uppercase tracking-widest hover:shadow-[0_0_30px_rgba(255,0,255,0.4)] disabled:opacity-20 disabled:cursor-not-allowed transition-all relative flex items-center justify-center gap-2"
                >
